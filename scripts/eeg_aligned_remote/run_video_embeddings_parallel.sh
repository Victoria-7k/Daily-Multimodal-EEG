...
if [ ! -d "$CACHE/openface" ]; then
  echo "missing 2xROI cache: $CACHE/openface" >&2
  exit 2
fi
...
run_variant() {
  ...
  local aug=$2
  local encoder=$3
  echo "variant=$route aug=$aug device=$DEVICE batch=$BATCH_SIZE frames=$NUM_FRAMES"
  cd "$REPO"
...
run_variant B0 none video_v4a_dinov2_2x_face_roi_mean_std_max
