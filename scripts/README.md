...
| `multilabel/` | 11-label fixed-token route/head 筛选、独立/共同 EEGPT partial-FT，以及共享多头与独立标量两条0814/0906结构矩阵及汇总。 | `48`、`92`-`102` |
| `eql_caf/` | EQL-CAF 5 x 2s temporal-token 探索线；当前 `59`-`62` 是 repeat-token smoke，不作为真实时序证据。 | `57`-`64` |
| `wear_only_fm/` | Wear-only foundation-model 表征路线、W3FM formal gate、post-gate 诊断和 ACC/NormWear 替换筛查。 | `65`-`72`、`89`-`90` |
...
5. 多情绪结构主流程先用 `92` 审计，再以固定 `A1_Wphysio_no_audio__eeg_eegpt_partial_ft_v1` 输入运行 `98`：0814一条窗口结构加0906的18条事件结构，同一 H1 11标签 head、两个协议、三个下游 seed；`99` 输出结构 × 情绪表。`93 --seeds 240800` 独立生成22份标签专属 EEGPT token，供后续 EEG 替换实验使用。历史 `48` fixed-token baseline、`95`/`96` embedding-route/head 筛查及 `97` 专属 EEG fusion 入口保留，不混入结构主表。
...
7. Wear-only FM 路线先用 `scripts/wear_only_fm/65_run_wear_only_fm_phase1.py --mode stage` 生成 staged inputs，再用 `--mode smoke` 和 `--mode batch` 缓存外部 frozen encoder embeddings；Phase 2/3 分别用 `66`、`67`、`68`，post-gate 诊断用 `69`-`72`，ACC handcrafted 替换筛查用 `89`，三模态全接 NormWear 的诊断筛查用 `90`。
