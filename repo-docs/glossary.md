...
| `within_subject_day` | 旧版同被试同日窗口级 holdout；窗口可分别进入 train、val、test，位于 `/vePFS-0x0d/DailyEEG/splits_new/within_subject_day` | [split 标识](references/data-contracts.md#date_in_order-split-标识) |
| `date_in_order` | 严格同一被试跨日期 held-out-day 协议；每个 subject-day 为不可拆分单位，按日期顺序进入训练、验证、测试，位于 aligned root 的 `outputs/splits/date_in_order` | [split 标识](references/data-contracts.md#date_in_order-split-标识) |
