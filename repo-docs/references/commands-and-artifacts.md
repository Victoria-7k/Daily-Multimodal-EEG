...
阶段 8/9/10 的入口已在本地合成数据和服务器 `wzw` 工作目录中验证通过。服务器阶段 10 全量对照显示 `modality_token_attention` 相比 baseline full test RMSE 从 `0.8756` 降到 `0.6968`，判定为 `accepted`。阶段 11/12 的本地测试覆盖了真实 embedding 契约、失败清单、缓存 key、四模态 cache 记录和缺失源失败语义；服务器阶段 12 使用 `window_index_with_video_audio.jsonl` 中 10 条完整且带 `video_candidates` 的窗口验证通过，EEG、Wear、Face、Audio 均为 `ready=10, missing=0, failures=0`，失败清单为 `[]`。阶段 13 到阶段 17 已在服务器 `lzs` 环境完成单被试验证：Audio wav2vec2、EEGPT、raw face、Wear sequence 和 all-real 打包均跑通 `sub-12` 25 个窗口，四模态 all-real 输出 shape 均为 `(25, 256)`，`modality_mask.sum(axis=0)=[25, 25, 25, 25]`，失败清单为 `[]`。阶段 18 入口已本地和服务器测试通过；服务器 `sub-12` smoke ablation 正确记录 `subject_split_incomplete`，全量和最终 ablation 结论后续统一执行。
