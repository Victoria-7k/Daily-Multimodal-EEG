...
## `date_in_order` split 标识

`within_subject_day` 保留为与协作者共享的旧版窗口级协议名称。严格的 held-out-day 划分统一命名为 `date_in_order`；新运行必须读取 `<aligned-root>/outputs/splits/date_in_order`，并在 manifest 中记录该 split root。

| Split 路径 | 划分单位与含义 | train / val / test 窗口数 | 重叠审计 |
| --- | --- | --- | --- |
| `<aligned-root>/outputs/splits/date_in_order` | 按 `subject-day pair` 整体划分；每个被试按日期排序，约 60% 日期训练、20% 验证、20% 测试 | `17135 / 5658 / 6026` | train/val/test 的 subject-day 与 event overlap 均为 `0` |
| `/vePFS-0x0d/DailyEEG/splits_new/within_subject_day` | 同被试同日窗口级 holdout；保留为可用的宽松协议 | `17243 / 5708 / 5868` | train-val、train-test、val-test 均共享全部 `150` 个 subject-day；train-val 与 val-test 各共享 `107` 个 EMA event |

`within_subject_day` 与 `date_in_order` 回答不同问题，结果须独立报告。历史输出中曾把 `date_in_order` 记录为 `within_subject_day`，这些输出保留原路径与 metadata 作为 provenance。构建规则见 [日期顺序 split 构建入口](../../scripts/archive_legacy/33_build_strict_within_subject_day_split.py)，overlap 记录见 [split audit](../../outputs/server_sync/eegpt_centered_improvement/split_audit_subject_day.json)。
