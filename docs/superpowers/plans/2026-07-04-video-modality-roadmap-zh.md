...
执行记录，2026-07-06 A3 停止：A1 和 A2 已完成，并已按 train-only override 协议先行分析。paired embedding audit 显示 A1/A2 会把同一 window 的 embedding 从 A0 推到几乎正交的位置（`cosine` 接近 0，`L2` 接近 `sqrt(2)`），这解释了 subject/session probe 下降但 fatigue downstream 明显变差的现象。基于该证据，A3 在完成前停止。服务器 PID `3403028`（A3 extractor）、`2211748`（A1/A2/A3 generator）和 `1779027`（等待 A3 的 A0-A3 eval runner）已于 2026-07-06 23:00 Asia/Shanghai 停止。A3 当时约 63.9%，但未写出最终 A3 `.npz`。后续只保留已完成的 A1/A2 artifacts 与 A0-A2 reports，不等待也不引用 A3 结果。

执行记录，2026-07-06 Weak-Aug：新增弱增强 profile `v4d_weak_color_brightness_contrast`，用于更温和的 V4d 分支。该 profile 只保留 weak brightness、weak contrast、weak color jitter，去掉 grayscale、blur、crop/scale jitter。目标不是把 subject/session probe 从约 `0.95` 打到 `0.07`，而是温和降到约 `0.60-0.80`，同时希望 LOSO/S4/S2 提升，S1 基本保持。本地和服务器 focused tests 已通过；服务器 1-window smoke 写出 `outputs/embeddings/video_v4d_weak_upper_body_smoke1_train_embeddings.npz`，quality flags 符合预期。当前 full train-only Weak-Aug extraction 以及后续 paired audit/downstream/probe evaluation 已在 `ncc_serve_4090` 启动，runner PID 为 `380378`，extractor PID 为 `380420`；输出将写入 `outputs/embeddings/video_v4d_weak_upper_body_train_embeddings.npz`、`outputs/reports/video_v4d_weak/`、`outputs/reports/video_variants/v4d_weak_train_only/` 和 `outputs/reports/video_probes/v4d_weak_train_only/`。
